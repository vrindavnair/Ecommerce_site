...
export const productFilterController=async(req,res)=>{
    try {
        ...
        if(radio.length) args.price={$gte: radio[0], $lte:radio[1]}; //gte->greater than equal to  lte->less than eqal to
        ...
        res.status(400).send({
            success:false,
            message:"Error white filtering the products"
        })
...
export const searchProductController = async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: 'Error in search product API' });
    }
};
